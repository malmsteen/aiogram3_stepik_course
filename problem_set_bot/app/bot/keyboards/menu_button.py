...
def get_main_menu_commands(i18n: dict[str, str], role: UserRole):
    if role == UserRole.USER:
        return [
            ...
            BotCommand(command="/sections", description="Выбор темы"),
            BotCommand(command="/choose", description="Отбор задач"),
        ...
        return [
            BotCommand(command="/start", description=i18n.get("/start_description")),
            BotCommand(command="/help", description=i18n.get("/help_description")),
            BotCommand(command="/sections", description="Выбор задач по теме"),
            BotCommand(command="/choose", description="Отбор задач"),
            BotCommand(command="/cart", description="Отобранные задачи"),
            BotCommand(command="/problems", description="Выбор конкретных задач"),
            BotCommand(command="/addanswer", description="Отправить ответ"),
