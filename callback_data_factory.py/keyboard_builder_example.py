...
builder.button(
    text="Категория 1",
    callback_data=GoodsCallbackFactory(category_id=1, subcategory_id=0, item_id=0),
)
# Добавляем вторую кнопку в билдер
builder.button(
    text="Категория 2",
    callback_data=GoodsCallbackFactory(category_id=2, subcategory_id=0, item_id=0),
)
...
async def process_start_command(message: Message):
    await message.answer(text="Вот такая клавиатура", reply_markup=builder.as_markup())
...
async def process_category_press(
    callback: CallbackQuery, callback_data: GoodsCallbackFactory
):
